...
// handleSyncToken returns the sync-board token, generating one on first
// request. Behind the normal access gate: a caller who cannot already reach
// the authenticated API has no business minting a credential.
//
// POST with {"rotate": true} issues a new token and invalidates the old one,
// which is the way to revoke a machine that should no longer sync.
func handleSyncToken(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == http.MethodPost {
		...
		// An empty body is a valid "just give me the token" request.
		_ = json.NewDecoder(http.MaxBytesReader(w, r.Body, 4096)).Decode(&req)
		rotate = req.Rotate
	}

	cfg := loadConfig()
	if cfg.SyncToken == "" || rotate {
		tok, err := randomAccessToken()
		if err != nil {
			http.Error(w, "Unable to generate sync token", http.StatusInternalServerError)
			return
		}
		cfg.SyncToken = tok
		saveConfig(cfg)
	...
	w.Header().Set("Content-Type", "application/json")
	_ = json.NewEncoder(w).Encode(map[string]string{"sync_token": cfg.SyncToken})
}
