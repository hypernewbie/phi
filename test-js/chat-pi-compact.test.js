...
describe('compact robust A — keep + file on-demand with id dedup', () => {
    ...
    it('keeps old history dimmed with divider, not blank, after compact', () => {
        const view = createReviewTranscriptView(root, { title: 'Pi', coder: 'pi', mode: 'structured', windowSize: 100, pageSize: 10 });
        const old = Array.from({ length: 5 }, (_, i) => mkMsg(`old-${i}`, i % 2 === 0 ? 'user' : 'assistant', `old ${i}`));
        view.setStructuredMessages(old, '', new Map());
        expect(countBlocks(root, '.user-message')).toBeGreaterThan(0);
        // snapshot before compact
        view.setCompactSnapshot({ messages: old, from: old.length, kept: 2, summary: 'compact summary', at: Date.now() });
        const live = [mkMsg('new-1', 'assistant', 'new 1'), mkMsg('new-2', 'user', 'new 2')];
        view.setStructuredMessages(live, '', new Map());
        // Should show old dimmed + divider + live, not blank
        expect(root.textContent).not.toContain('Start of loaded history');
    ...
    it('dedups by id — hi hi duplication is impossible', () => {
        ...
        const old = [hi];
        view.setCompactSnapshot({ messages: old, from: 1, kept: 1, summary: null, at: Date.now() });
        // live also contains same hi (as would happen if we naively re-read jsonl after compact)
        const liveWithDup = [mkMsg('hi-id', 'user', 'hi'), mkMsg('new-2', 'assistant', 'after')];
        view.setStructuredMessages(liveWithDup, '', new Map());
        const userBlocks = [...root.querySelectorAll('.user-message')].map((n) => n.textContent);
    ...
    it('window slides over combined [snapshot + divider + live] and Load older works', () => {
        const view = createReviewTranscriptView(root, { title: 'Pi', coder: 'pi', mode: 'structured', windowSize: 2, pageSize: 1 });
        const old = Array.from({ length: 5 }, (_, i) => mkMsg(`o-${i}`, 'user', `o-${i}`));
        view.setCompactSnapshot({ messages: old, from: 5, kept: 1, summary: null, at: Date.now() });
    ...
    it('new incognito: loadCompactSnapshotFromFile on demand fetches old pre-compact turns with id dedup', async () => {
        ...
        const fakeJsonl = [
            JSON.stringify({ type: 'message', message: mkMsg('old-1', 'user', 'old 1') }),
            JSON.stringify({ type: 'message', message: mkMsg('old-2', 'user', 'old 2') }),
            JSON.stringify({ type: 'compaction', summary: 'file summary', firstKeptEntryId: 'live-1' }),
            JSON.stringify({ type: 'message', message: mkMsg('live-1', 'assistant', 'live 1') }),
            // duplicate live id after compaction should be ignored
            JSON.stringify({ type: 'message', message: mkMsg('live-1', 'assistant', 'live 1 dup') }),
        ].join('\n');
        const fetchImpl = vi.fn(async (url) => {
            if (String(url).includes('/api/sessions')) {
                return { ok: true, json: async () => ({ sessions: [{ path: '/tmp/fake.jsonl' }] }) } as any;
            }
            if (String(url).includes('/api/fs/read')) {
                return { ok: true, text: async () => fakeJsonl } as any;
            }
            return { ok: false } as any;
        });
        const loaded = await view.loadCompactSnapshotFromFile({ cwd: '/work/demo', fetchImpl });
    ...
    it('file on-demand merges without hi hi when live already has hi', async () => {
        ...
        const fakeJsonl = [
            JSON.stringify({ type: 'message', message: mkMsg('hi-id', 'user', 'hi old') }),
            JSON.stringify({ type: 'compaction', summary: 's' }),
            JSON.stringify({ type: 'message', message: mkMsg('hi-id', 'user', 'hi') }),
        ].join('\n');
        const fetchImpl = vi.fn(async (url) => {
            if (String(url).includes('/api/fs/read')) return { ok: true, text: async () => fakeJsonl } as any;
            return { ok: false, json: async () => ({ sessions: [] }) } as any;
        });
        // Direct file read via explicit path
        const loaded = await view.loadCompactSnapshotFromFile({ sessionPath: '/tmp/x.jsonl', fetchImpl });
