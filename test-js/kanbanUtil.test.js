...
describe('extractVikunjaError', () => {
    it('returns status-based fallback for empty body', () => {
        expect(extractVikunjaError('', 500)).toBe('Request failed with status 500');
    });

    it('pulls the message from the single-message Vikunja envelope', () => {
        expect(extractVikunjaError('{"message":"method not allowed error","code":7}', 405))
            .toBe('method not allowed error');
    });

    it('flattens the per-field "messages" envelope', () => {
        expect(extractVikunjaError('{"messages":{"title":["can\'t be empty"],"bucket":["required"]}}', 400))
            .toBe("title: can't be empty; bucket: required");
    });

    it('falls back to a truncated, tag-stripped text on non-JSON (e.g. nginx HTML)', () => {
        const html = '<html><body><h1>502 Bad Gateway</h1><p>ohai</p></body></html>';
    ...
    it('truncates long messages to 240 chars + "..."', () => {
        ...
        expect(extractVikunjaError(`{"message":"${big}"}`, 500)).toMatch(/^x+\.\.\.$/);
        expect(extractVikunjaError(`{"message":"${big}"}`, 500).length).toBe(243);
    });
});
