...
describe('buildProxyUrl', () => {
    it('joins coordinator base and endpoint, URL-encoded', () => {
        expect(buildProxyUrl('http://localhost:7070', '/api/messages')).toBe(
            '/api/proxy?url=' +
                encodeURIComponent('http://localhost:7070/api/messages'),
        );
    });
    ...
    it('encodes query params in the endpoint', () => {
        ...
        expect(url).toBe(
            '/api/proxy?url=' +
                encodeURIComponent('http://host/search?q=a b&n=1'),
        );
