...
const bash = (over = {}) => ({ coder: 'bash', isDead: false, title: 'bash', paneId: 'p1', cwd: '/proj', ...over });
...
describe('findReusableShellTab', () => {
    const opts = (over = {}) => ({ useExistingTerminalTab: true, activeCWD: '/proj', ...over });
    ...
    it('returns the active tab when it is itself a usable shell (highest precedence)', () => {
        ...
        expect(findReusableShellTab([active, other], active, opts())).toBe(active);
    });
    ...
    it('finds an alive shell matching activeCWD when active tab is not a shell', () => {
        ...
        expect(findReusableShellTab([active, match], active, opts())).toBe(match);
    });

    it('matches CWD via normalizeCwd (backslashes + trailing slash)', () => {
        const match = bash({ paneId: 'm', cwd: 'C:\\proj' });
        const found = findReusableShellTab([match], null, opts({ activeCWD: 'C:/proj/' }));
    ...
    it('returns null when useExistingTerminalTab is off', () => {
        ...
        expect(findReusableShellTab([match], null, opts({ useExistingTerminalTab: false }))).toBeNull();
    });

    it('returns null when activeCWD is empty', () => {
        const match = bash({ cwd: '/proj' });
        expect(findReusableShellTab([match], null, opts({ activeCWD: '' }))).toBeNull();
    });
