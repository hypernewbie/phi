...
//   3. Tablet diff slide-over: 769–1024px the diff panel is the drawer,
//      not a side-by-side column.
//
...
describe('isCompactViewport', () => {
    ...
    it('keeps desktop layout on wide viewports, touch or not', () => {
        ...
        // An 820px iPad is a touch device but NOT compact: it keeps the
        // side-by-side tablet layout (sidebar column + diff slide-over).
...
describe('style.css tablet diff slide-over', () => {
    it('769–1024px turns the diff panel into the slide-over drawer', () => {
        const m = CSS.match(
            /@media \(min-width: 769px\) and \(max-width: 1024px\) \{[\s\S]*?\n\}/,
        );
        expect(m, 'tablet media block not found').toBeTruthy();
        const block = m[0];
        ...
        // Only the diff handle hides; the sidebar keeps its resizer.
        expect(block).toMatch(/#right-resize-handle\s*\{[^}]*display:\s*none/);
        expect(block).not.toContain('#left-resize-handle');
        expect(block).not.toContain('.resize-handle {');
    });
});
