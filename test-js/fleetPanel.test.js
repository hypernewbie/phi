...
describe('App.renderFleetPanel', () => {
    ...
    it('hides the panel when statuses is empty', () => {
        const a = makeApp();
        a.renderFleetPanel([]);
        expect(document.getElementById('fleet-panel').style.display).toBe(
            'none',
        );
    });
    ...
    it('marks rows unreachable when peer is not reachable, disables click', () => {
        ...
        expect(row.querySelector('.fleet-peer-meta').textContent).toBe(
            'offline',
        );
        // Stale badge appears because there is an unreachable peer
        expect(
            document.getElementById('fleet-stale-badge').style.display,
        ).not.toBe('none');
    });
    ...
    it('renders multiple rows and clears stale badge when all reachable', () => {
        ...
        expect(document.getElementById('fleet-stale-badge').style.display).toBe(
            'none',
        );
    });
