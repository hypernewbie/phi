import type { StructuredMessage, Segment } from './render.js';
import type { StructuredMessageSource } from './message-buffer.js';
...
export interface SearchMatch extends SearchCorpusEntry {
    start: number;
    end: number;
    occurrence: number;
    snippet: string;
...
export interface PiSearchView {
    revealMessage(index: number): boolean;
    markSearchMatch(index: number, query: string, occurrence: number): boolean;
    clearSearchMarks(): void;
}

export interface PiSearchController {
    setSource(source: StructuredMessageSource): void;
    toggle(): boolean;
    open(): void;
    close(): boolean;
    isOpen(): boolean;
...
export interface PiSearchControllerOptions extends PiSearchView {
    root: HTMLElement;
}
...
export function flatTextFromMessage(message: StructuredMessage): string {
...
export function createPiSearchController(
    options: PiSearchControllerOptions,
): PiSearchController {
    ...
    let source: StructuredMessageSource = {
        length: 0,
        slice: () => [] as readonly StructuredMessage[],
    };
    let corpus: SearchCorpusEntry[] = [];
    let matches: SearchMatch[] = [];
    let activeMatch = 0;
    let query = '';
    let debounceTimer: ReturnType<typeof setTimeout> | null = null;

    const isOpen = (): boolean => !bar.classList.contains('hidden');
