...
import type { ToolResultEntry } from './message-buffer.js';
...
export interface ToolCallInput {
    ...
    status: ToolStatus;
    /** Result text (trimmed) once the tool has settled. */
    output?: string;
    /** Validated Pi edit display diff. */
    diff?: string;
}
 ...
 */
export function validatedToolDiff(
...
function appendIntraLineDiffRows(
    container: HTMLElement,
...
function appendDiffRows(container: HTMLElement, diffText: string): void {
...
function appendHeader(
    ...
    const addPath = (raw: string, extra?: HTMLElement | null): void => {
 ...
 */
export function renderToolExecution(input: ToolCallInput): HTMLElement {
    ...
    const appendPlainOutput = (text: string): void => {
    ...
    const header = wrapper.querySelector<HTMLElement>('.tool-header');
    if (header) {
        const toggle = (): void => {
            ...
            wrapper
                .querySelectorAll<HTMLElement>('.tool-output.expandable')
                .forEach((el) => {
