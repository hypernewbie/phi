...
export type BashStatus = 'pending' | 'success' | 'error';

export interface BashCallInput {
    id: string;
    command: string;
    output?: string;
    status: BashStatus;
 ...
 */
export function renderBashExecution(input: BashCallInput): HTMLElement {
    ...
    const syncHints = (): void => {
        const expandable = wrapper.querySelector<HTMLElement>(
            '.tool-output.expandable',
        );
        const expanded = expandable?.classList.contains('expanded') ?? false;
        wrapper
            .querySelectorAll<HTMLElement>('.expand-hint')
            .forEach((hint) => {
    ...
    if (
        ...
        footer.textContent = `(exit ${(details as { exitCode: number }).exitCode})`;
