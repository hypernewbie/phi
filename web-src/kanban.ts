...
import { buildFeatures, featureProgress, featureTimeline, type FeatureProgress } from './kanban-features.js';
...
export class KanbanManager {
    ...
    renderBoardLayout(container: HTMLElement, projects: any[], currentProject: any, kanbanView: any, bucketsWithTasks: any[] | null): void {
        ...
        container.querySelectorAll('.kanban-view-btn').forEach((button: Element) => {
            button.addEventListener('click', () => {
                this.boardMode = (button as HTMLElement).dataset.kanbanMode === 'features' ? 'features' : 'board';
                this.renderBoardLayout(container, projects, currentProject, kanbanView, bucketsWithTasks);
            });
        });
        ...
        container.querySelectorAll('.kanban-feature-row').forEach((row: Element) => {
        ...
        container.querySelectorAll('.kanban-feature-done-btn').forEach((button: Element) => {
            button.addEventListener('click', async (event) => {
                event.stopPropagation();
                const task = this.taskCache[(button as HTMLElement).dataset.taskId!];
                if (!task) return;
                const progress = featureProgress(task);
                if (!task.done && progress.completed < progress.total && !confirm(`Mark "${task.title}" done with ${progress.total - progress.completed} unfinished subtask${progress.total - progress.completed === 1 ? '' : 's'}?`)) return;
                const doneButton = button as HTMLButtonElement;
    ...
    renderDetailPanelContent(panel: HTMLElement, task: any, cardEl: HTMLElement, container: HTMLElement): void {
        ...
        // Wire events
        const toggleBtn = panel.querySelector('#kdp-desc-toggle-btn') as HTMLButtonElement;
        const descView = panel.querySelector('#kdp-description-view') as HTMLElement;
        const descInput = panel.querySelector('#kdp-description') as HTMLTextAreaElement;
        ...
        panel.querySelector('.kdp-save-btn')!.addEventListener('click', async () => {
            ...
            try {
                ...
                const newDescription = (panel.querySelector('#kdp-description') as HTMLTextAreaElement).value;
