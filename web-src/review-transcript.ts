...
import type {
    ...
    ToolResultEntry,
} from './chat-pi/message-buffer.js';
import { renderBashExecution, type BashStatus } from './chat-pi/bash-render.js';
import {
    renderToolExecution,
    validatedToolDiff,
...
function asSource(
    messages: readonly StructuredMessage[] | StructuredMessageSource,
): StructuredMessageSource {
    if (typeof (messages as StructuredMessageSource).slice === 'function') {
        return messages as StructuredMessageSource;
    }
    return arrayAsSource(messages as readonly StructuredMessage[]);
}
...
export interface ReviewTranscriptOptions {
    title: string;
    coder: string;
    status?: string;
    refresh?: boolean;
    ...
    /**
     * `structured` consumes the Pi-TUI-flavoured chat-pi segment shape
     * (`setStructuredMessages`) and renders `.user-message`,
     * `.assistant-message`, `.tool-execution` blocks. `legacy` (default)
     * keeps the bubble layout used by `web-src/sessions.ts`.
     */
    mode?: 'legacy' | 'structured';
    /** Max messages held in the DOM at once (structured mode only). */
    windowSize?: number;
    /** How many messages to prepend per scroll-up page (structured mode). */
    pageSize?: number;
}
...
export interface ReviewTranscriptView {
    ...
    setMessages(messages: ReviewTranscriptMessage[], partial?: string): void;
    showEmpty(): void;
    // Structured mode (chat-pi):
    ...
    /**
     * Narrow live-text update for streaming `text_delta` events. Mutates
     * (or lazily creates) only the single `.assistant-message.pi-streaming`
     * block; never calls `rebuildStructuredWindow`, markdown parsing,
     * tool lookup, cache-key creation, or `replaceChildren`. Empty
...
function appendThinkingText(target: HTMLElement, text: string): void {
    const pattern = /\*\*(.+?)\*\*/gs;
    let lastIndex = 0;
...
function unsupportedToolResultLabel(item: unknown): string {
...
function extractTextFromToolResult(message: InboundMessage): string {
    const content = message.content;
    if (typeof content === 'string') return content;
    const textForItem = (item: unknown): string => {
...
function collectValidatedToolDiffs(
    messages: readonly StructuredMessage[],
    toolResults: ReadonlyMap<string, ToolResultEntry>,
): Map<string, string> {
...
function buildUnsupportedContentBlock(label: string): HTMLElement {
...
function buildUserMessageBlock(
    segments: Array<
        { kind: 'text'; text: string } | { kind: 'unsupported'; label: string }
    >,
    copyText: ((text: string) => void) | undefined,
): HTMLElement {
    const block = document.createElement('div');
...
export function createReviewTranscriptView(
    root: HTMLElement,
    options: ReviewTranscriptOptions,
): ReviewTranscriptView {
    ...
    let activeBottom: HTMLElement | null = null;
    if (options.mode === 'structured') {
    ...
    let structuredMessages: StructuredMessageSource = {
        length: 0,
    ...
    function syncActivePromptMarkers(): void {
        for (const block of transcript.querySelectorAll<HTMLElement>(
        ...
        if (typeof activeTurn.promptOrigin === 'number') {
            const block = transcript.querySelector<HTMLElement>(
     ...
     */
    function findLiveAssistantBlock(): HTMLElement | null {
    ...
    function applyLiveToolOutputToHost(
        host: ParentNode,
        id: string,
        output: string,
    ): HTMLElement | null {
        const wrapper = Array.from(
            host.querySelectorAll<HTMLElement>('.tool-execution'),
        ).find((candidate) => candidate.id === `tool-call-${id}`);
        if (!wrapper) return null;
        wrapper.classList.add('pi-live-tool');
        let live = wrapper.querySelector<HTMLElement>('.pi-live-tool-output');
        if (!output) {
            if (live) live.remove();
        ...
        if (!live) {
            live =
                Array.from(
                    wrapper.querySelectorAll<HTMLElement>('.tool-output'),
     ...
     * with the latest text.
     */
    function setStructuredThinking(text: string): void {
        latestThinking = text;
        ...
        if (existing) {
            const content =
                existing.querySelector<HTMLElement>('.thinking-text');
    ...
    function setLiveToolOutput(id: string, text: string): void {
    ...
    function buildAssistantOrUserBlock(
        msg: StructuredMessage,
    ...
    function applyScrollAnchor(
        snap: { wasAtBottom: boolean; preHeight: number },
        delta: number,
    ): void {
        if (snap.wasAtBottom) {
            transcript.scrollTop = transcript.scrollHeight;
    ...
    const view: ReviewTranscriptView = {
        root,
        title,
        contentBody,
        transcript,
        ...
        appendNewer(count) {
            ...
            let anchor: Element | null = transcript.lastElementChild;
            while (
                anchor &&
