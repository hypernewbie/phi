...
export class MarkdownManager {
    ...
    pasteModal!: HTMLElement;
    pasteModalForm!: HTMLFormElement;
    pasteModalTitle!: HTMLElement;
    pasteModalHint!: HTMLElement;
    pasteModalContent!: HTMLTextAreaElement;
    pasteModalName!: HTMLInputElement;
    pasteModalDir!: HTMLElement;
    pasteModalDirLabel!: HTMLElement;
    pasteModalError!: HTMLElement;
    pasteModalCancel!: HTMLElement;
    pasteModalSave!: HTMLButtonElement;
    pasteModalClose!: HTMLElement;
    _pastePending: boolean;
    ...
    _setupEventListeners(): void {
        ...
        if (this.pasteModal) {
            this.pasteModal.addEventListener('click', (e) => {
                if (e.target === this.pasteModal) this._closePasteModal();
            });
        }
    ...
    async _copyMarkdownBlob(file: any): Promise<void> {
        try {
            const cwd = this.app.sessionsManager.activeCWD || '';
            const res = await fetch(
                `/api/markdown/file?path=${encodeURIComponent(file.path)}&cwd=${encodeURIComponent(cwd)}`,
    ...
    async _pasteFromSystemClipboard(): Promise<void> {
        ...
        if (!firstLine.startsWith('#')) {
            this.app.showToast(
                'Clipboard does not start with "# path/to/file.md"',
                {
                    type: 'error',
                    title: 'Markdown',
                },
        ...
        const cwd = this.app.sessionsManager.activeCWD || '';
        const doPaste = async (overwrite: boolean): Promise<Response> => {
        ...
        try {
            ...
            if (res.status === 409) {
                const ok = confirm(
                    `"${relPath}" already exists. Overwrite it?`,
                );
                if (!ok) return;
            ...
            this.app.showToast(`Paste failed: ${(e as Error).message}`, {
                type: 'error',
                title: 'Markdown',
            });
        }
    ...
    _readClipboardViaUserPaste(): Promise<string | null> {
...
body..."></textarea>
    ...
    async _submitPaste(): Promise<void> {
        ...
        try {
            ...
            this._setPasteError((e as Error).message || 'Paste failed');
            this._pastePending = false;
            this.pasteModalSave.disabled = false;
        }
    }

    _closePasteModal(): void {
        if (this._pastePending) return; // don't yank the modal mid-request
