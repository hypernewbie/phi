...
const NATIVE_POPOUT_PAGES: Record<string, string> = {
    config: '/config.html',
    help: '/md.html?page=help',
    changelog: '/md.html?page=changelog',
};

export function markDesktopView(): void {
    if (new URLSearchParams(location.search).get('desktop') === '1') {
        document.documentElement.setAttribute('data-phi-desktop', '');
    }
}

export function tryNative(kind: string, payload: unknown): boolean {
    const bridge = (window as any).__phiDesktop;
