...
export function validateDialogResponse(record, answer) {
...
export function dialogResponseFor(record, value, confirmed) {
    if (record.method === 'confirm') {
...
function connectedFocus(element) {
    return element?.isConnected ? element : null;
}
function closeAnnouncement(reason) {
    switch (reason) {
        case 'timeout':
...
export function createPiDialogController(options) {
    const entries = new Map();
    const isActive = options.isActive ?? (() => true);
    let activeId = null;
    let destroyed = false;
    const focusables = (entry) =>
        Array.from(
    ...
    const setDisabled = (entry, disabled) => {
    ...
    const focusInitial = (entry) => {
        const target =
            entry.record.method === 'select'
                ? entry.dialog.querySelector('.pi-extension-dialog-option')
                : entry.record.method === 'confirm'
                  ? entry.dialog.querySelector('.pi-extension-dialog-cancel')
    ...
    const removeEntry = (id, reason, restoreFocus) => {
    ...
    const submit = (entry, answer) => {
        ...
        void options
            ...
            .catch((error) => {
                ...
                setError(
                    entry,
                    `Dialog response failed: ${error instanceof Error ? error.message : String(error)}`,
                );
                const cancel = entry.dialog.querySelector(
                    '.pi-extension-dialog-cancel',
                );
                cancel?.focus({ preventScroll: true });
            });
    };
    const onDialogKeydown = (entry, event) => {
        if (entry.resolving) {
    ...
    const addButton = (entry, className, label, handler) => {
    ...
    return {
        setDialogs(dialogs) {
            ...
            if (!activeId && entries.size > 0)
                activate(entries.keys().next().value);
            showActive();
        },
        upsertDialog,
