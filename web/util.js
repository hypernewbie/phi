...
export function setServerHostOverride(label) {
...
export function clampPanelWidth(px, cap, viewportWidth = typeof window !== 'undefined'
    ? window.innerWidth
    : cap.max) {
...
export function terminalPreferredFontSize(value) {
    const size = Number(value);
    return Number.isFinite(size) && size >= 8 && size <= 32
        ? size
        : TERMINAL_DEFAULT_FONT_SIZE;
}
export function responsiveTerminalFontSize(preferredSize, renderedSize, renderedColumns, targetColumns = TERMINAL_TARGET_COLUMNS) {
...
export const TERMINAL_LINK_REGEX = /(?:(?:https?:\/\/|mailto:)[^\s()<>]+|www\.[^\s()<>]+\.[^\s()<>]+)(?:\([\w\d]+\)|(?:[^\s`!()[\]{};:'".,<>?«»“”‘’]|(?:\b|\/)))/gi;
...
export function findTerminalLineLinks(buffer, bufferLineNumber) {
    if (!buffer || typeof buffer.getLine !== 'function')
        return [];
    const y = bufferLineNumber;
    if (!Number.isFinite(y) || y < 1 || y > buffer.length)
        return [];
    let startY = y;
    while (startY > 1) {
...
export function installTerminalLinkProvider(term, onActivate) {
    if (!term || typeof term.registerLinkProvider !== 'function')
        return;
    return term.registerLinkProvider({
        provideLinks: (bufferLineNumber, callback) => {
            const links = findTerminalLineLinks(term.buffer?.active, bufferLineNumber);
