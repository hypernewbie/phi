...
export class KanbanManager {
    ...
    async openBoard() {
        ...
        if (existing) {
            ...
            if (existing.termContainer &&
                !existing.termContainer.querySelector('.kanban-toolbar, .kanban-login-form')) {
    ...
    async initTabContainer(container, isAutoRetry = false) {
        ...
        if (vault.url)
            localStorage.setItem('vikunja_url', vault.url);
        const savedPw = vault.password;
        const urlVal = (vault.url ||
            localStorage.getItem('vikunja_url') ||
            'http://charon:3456').replace(/\/$/, '');
        const userVal = vault.username || localStorage.getItem('vikunja_username');
    ...
    async getSavedVaultCredentials() {
        try {
    ...
    renderLoginForm(container) {
        ...
        loginBtn.addEventListener('click', async () => {
            const urlInput = container.querySelector('#kanban-url-input').value
                .trim()
                .replace(/\/$/, '');
    ...
    async loadBoard(container, { showSpinner = true, isAutoRetry = false, } = {}) {
        ...
        try {
            ...
            let currentProject = projects.find((p) => String(p.id) === String(selectedProjectId));
            if (!currentProject) {
            ...
            try {
                projectTasks =
                    (await this.apiGet(`/projects/${selectedProjectId}/tasks?per_page=500&expand=subtasks`)) || [];
            }
            catch (err) {
                    ...
                    if (bucket.tasks) {
                        bucket.tasks = bucket.tasks.map((boardTask) => {
                            ...
                            const task = hierarchyTask?.related_tasks
                                ? {
                                    ...boardTask,
                                    related_tasks: hierarchyTask.related_tasks,
                                }
                                : boardTask;
                ...
                <div class="kanban-toolbar">
                    <div class="toolbar-left">
                        ...
                        <select id="kanban-project-select" class="kanban-select">
                            ${projects.map((p) => `<option value="${p.id}" ${String(p.id) === String(currentProject.id) ? 'selected' : ''}>${this.escapeHtml(p.title)}</option>`).join('')}
                        </select>
    ...
    renderStatsView() {
        ...
        const formatDate = (date) => new Date(`${date}T00:00:00Z`).toLocaleDateString(undefined, {
            month: 'short',
            day: 'numeric',
            year: 'numeric',
            timeZone: 'UTC',
    ...
    initStatsCharts(container) {
        ...
        const burnupLabels = burnup.length > 0
            ? burnup.map((point) => this.chartDateLabel(point.date))
            : ['Today'];
        const filed = burnup.length > 0
            ? burnup.map((point) => point.filed)
            : [stats.totalTasks];
        const completed = burnup.length > 0
                ...
                datasets: [
                    {
                        type: 'bar',
                        label: 'Completed',
                        data: stats.dailyCompletions.map((day) => day.completed),
                    ...
                    },
                    {
                        ...
                        data: stats.dailyCompletions.map(() => stats.velocityPerDay),
                        borderColor: accent,
                        borderDash: [4, 3],
                        pointRadius: 0,
                        borderWidth: 1.5,
    ...
    renderFeatureTimeline(subtasks) {
        ...
        const x = (index) => timeline.length === 1
            ? width
            : (index / (timeline.length - 1)) * width;
        const y = (value) => height - (value / maximum) * height;
        const points = (field) => timeline
            .map((point, index) => `${x(index).toFixed(1)},${y(point[field]).toFixed(1)}`)
            .join(' ');
        ...
        lists.forEach((list) => {
            ...
            window.Sortable.create(list, {
                ...
                onEnd: async (evt) => {
                    ...
                    const oldBucketId = evt.from.dataset
                        .bucketId;
                    const newBucketId = evt.to.dataset
                        .bucketId;
                    if (oldBucketId === newBucketId)
                        return;
                    // Optimistically update card done styling
                    const targetBucket = (this.buckets || []).find((b) => String(b.id) === String(newBucketId));
                    ...
                    try {
                        ...
                        // Revert done styling
                        const originalBucket = (this.buckets || []).find((b) => String(b.id) === String(oldBucketId));
    ...
    taskDescriptionPreview(description) {
        return (this.sanitizeTaskDescription(description) ||
    ...
    async setTaskDone(task, done) {
        ...
        if (done)
            optimistic.done_at = task.done_at || new Date().toISOString();
        // Vikunja's UPDATE endpoint is POST /tasks/{id} (not PUT — only CREATE is PUT).
        const response = await this.applyTaskMutation(task.id, optimistic, () => this.apiPost(`/tasks/${task.id}`, payload));
    ...
    async saveTaskDetail(task, formData, cardEl, container) {
        ...
        await this.applyTaskMutation(task.id, optimistic, () => this.apiPost(`/tasks/${task.id}`, payload));
    ...
    bindBoardDelegates(container) {
        if (this._boardDelegateHost === container)
            return;
        this.unbindBoardDelegates();
    ...
    bucketIdOf(taskId) {
        ...
        for (const bucket of this.buckets || []) {
            if ((bucket.tasks || []).some((t) => String(t.id) === id))
    ...
    removeTaskState(taskId) {
        ...
        for (const bucket of this.buckets || []) {
            if (!bucket.tasks)
                continue;
            bucket.tasks = bucket.tasks.filter((t) => String(t.id) !== id);
        }
    }
    addTaskState(task, bucketId) {
        if (!task || task.id == null)
            return;
        this.removeTaskState(task.id);
        this.taskCache[String(task.id)] = task;
        const bucket = (this.buckets || []).find((b) => String(b.id) === String(bucketId));
        if (!bucket)
            return;
        if (!bucket.tasks)
            bucket.tasks = [];
    ...
    insertCard(task, bucketId) {
        if (!this.boardContainer || !task)
            return;
    ...
    syncColumnCounts() {
        if (this.boardContainer)
            this.updateColumnCounts(this.boardContainer);
    }
    // ---- filtering -----------------------------------------------------
    ...
    async applyTaskMutation(taskId, patch, commit) {
        const id = String(taskId);
        ...
        try {
            ...
            if (this._taskWrites.get(id) === run)
                this._taskWrites.delete(id);
        }
    }
    async runTaskMutation(id, patch, commit) {
    ...
    // ---- board-level refresh -------------------------------------------
    captureBoardUi(container) {
        const scrolls = {};
        container.querySelectorAll('.kanban-cards-list').forEach((list) => {
            scrolls[list.dataset.bucketId] = list.scrollTop;
        });
    ...
    restoreBoardUi(container, snapshot) {
        if (!snapshot)
            return;
        ...
        const wrapper = container.querySelector('.kanban-columns-wrapper');
        if (wrapper)
            wrapper.scrollLeft = snapshot.wrapperScrollLeft || 0;
        const input = container.querySelector('#kanban-search-input');
    ...
    setBoardSyncing(on) {
        ...
        if (indicator)
            indicator.classList.toggle('is-syncing', on);
    }
    ...
    async deleteTask(taskId, _container) {
    ...
    async deleteBucket(bucketId, container) {
        ...
        this.buckets = (this.buckets || []).filter((b) => String(b.id) !== String(bucketId));
        if (container)
            await this.refreshBoard(container);
    }
    // ============================================================
    ...
    async _wireLabelPicker(panel, task, container) {
        ...
        try {
            ...
            const opts = ['<option value="">Add label…</option>'].concat(all
    ...
    async removeLabelFromTask(taskId, labelId) {
        ...
        if (this.taskCache[taskId] &&
            ...
            this.taskCache[taskId].labels = this.taskCache[taskId].labels.filter((l) => String(l.id) !== String(labelId));
        }
    }
    ...
    async reauthenticate() {
        ...
        this._reauthPromise = (async () => {
            try {
                ...
                if (creds.url)
                    localStorage.setItem('vikunja_url', creds.url);
                const password = creds.password;
                const url = (creds.url ||
                    localStorage.getItem('vikunja_url') ||
                    '').replace(/\/$/, '');
                const username = creds.username || localStorage.getItem('vikunja_username');
