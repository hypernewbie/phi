...
// transition history.
export function cumulativeTimeline(tasks) {
...
export function taskStats(tasks, now = new Date(), velocityWindowDays = 28) {
    ...
    tasks.forEach((task) => {
        ...
        if (created && dailyByDate.has(created))
            createdInWindow += 1;
        if (!task.done)
            return;
        completedTasks += 1;
    ...
    const projectedCompletionDate = estimatedDaysRemaining == null
        ? null
        : utcDate(new Date(today.getTime() +
...
export function featureStats(features, now = new Date(), velocityWindowDays = 28) {
    ...
    return {
        totalFeatures,
        completedFeatures,
        featurePercent: totalFeatures === 0
            ? 0
            : Math.round((completedFeatures / totalFeatures) * 100),
        totalSubtasks,
        completedSubtasks,
        subtaskPercent: totalSubtasks === 0
            ? 0
