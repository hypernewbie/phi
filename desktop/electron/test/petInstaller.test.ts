...
const validArchive = (includeLicense = true): Uint8Array =>
  gzipSync(
    tar(
...
describe('installPet', () => {
  ...
  it.each([
    ...
    const root = tempDir();
    await expect(
      installPet(depsFor(root, gzipSync(archive))),
    ).rejects.toThrow();
  ...
  it('rejects an incomplete package and cleans staging', async () => {
    const root = tempDir();
    await expect(
      installPet(depsFor(root, validArchive(false))),
  ...
  it('short-circuits an idempotent install without fetching or rewriting', async () => {
    ...
    await expect(installPet(second)).resolves.toEqual({
      root: path.join(root, 'pet', '1.2.3'),
    });
  ...
  it('rejects a bad version or repository before fetching', async () => {
    ...
    await expect(installPet(badVersion)).rejects.toThrow(
    ...
    const badRepo = depsFor(
      ...
      validArchive(),
      '1.2.3',
      'hypernewbie/phi/releases',
    );
    await expect(installPet(badRepo)).rejects.toThrow(
