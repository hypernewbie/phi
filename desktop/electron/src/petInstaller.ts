...
function completePetRoot(root: string): boolean {
 try {
  if (!REQUIRED_FILES.every((file) => statSync(path.join(root, file)).isFile()))
   return false;
...
function parseArchive(bytes: Uint8Array, staging: string): void {
 ...
 while (offset + BLOCK_SIZE <= bytes.length) {
  ...
  if (header.every((byte) => byte === 0)) {
   if (
    offset + BLOCK_SIZE > bytes.length ||
    !bytes.subarray(offset, offset + BLOCK_SIZE).every((byte) => byte === 0)
   ) {
  ...
  if (offset + payloadBlocks * BLOCK_SIZE > bytes.length)
   throw new Error("pet archive is truncated");
 ...
 if (
  !ended ||
  (offset !== bytes.length && bytes.subarray(offset).some((byte) => byte !== 0))
 ) {
  throw new Error("pet archive is truncated");
 }
}
...
export async function installPet(
 ...
 mkdirSync(staging);
 try {
  const url = `https://github.com/${deps.repo}/releases/download/v${deps.appVersion}/phi-pet-${deps.appVersion}.tar.gz`;
  const archive = gunzipSync(await deps.fetchBytes(url));
  parseArchive(archive, staging);
  if (!completePetRoot(staging))
